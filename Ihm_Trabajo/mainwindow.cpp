...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_loginController(nullptr),
      ...
      m_profileController(nullptr), m_chartWidget(nullptr),
      m_coordLabel(nullptr), m_zoomLabel(nullptr), m_angleLabel(nullptr),
      m_toolGroup(nullptr), m_strokePopup(nullptr), m_profileView(nullptr),
      m_resultsView(nullptr), m_quizView(nullptr) {
  ui->setupUi(this);
...
void MainWindow::setupToolbar() {
  ...
  modeMenu->setStyleSheet(
      "QMenu { background-color: #1a1a1a; color: white; border: 1px solid "
      "#333; } QMenu::item:selected { background-color: #e94560; }");

  QAction *actPan = modeMenu->addAction(
      createWhiteIcon(":/resources/icons/hand.svg"), "Moverse (Mano)");
  QAction *actSelect = modeMenu->addAction(
      createWhiteIcon(":/resources/icons/cursor.svg"), "Seleccionar");
  ...
  // Generalmente repite la última acción o abre menú. Con MenuButtonPopup, el
  // click ejecuta defaultAction. Vamos a hacer que el click principal active la
  // herramienta que muestra el icono actualmente. Pero
  // QToolButton::MenuButtonPopup separa el click del menú.
  connect(btnMode, &QToolButton::clicked, this, [=]() {
    // Ejecutar la herramienta activa visualmente
    // Esto requiere saber cuál es estado actual. Lo podemos deducir del
    // controller.
  ...
  // Insertar en toolbar (reemplazando toolPan viejo si es posible, o insertando
  // al inicio)
  if (auto layout =
          ui->centralwidget->findChild<QVBoxLayout *>("toolbarVLayout")) {
  ...
  // Añadir al grupo para exclusividad visual (aunque el QToolButton no es
  // checkable igual que los otros, lo haremos checkable para que se ilumine
  // cuando activas Pan o Select)
  btnMode->setCheckable(true);
  btnMode->setChecked(true); // Default Pan

  // El grupo de herramientas necesita IDs únicos.
  // Como Pan y Select son modos distintos, pero comparten botón... esto es
  // complejo para QButtonGroup. Solución: No añadir este botón al grupo
  // principal de forma estándar, O gestionar su estado visual manualmente en
  // updateToolButtonStates. Vamos a gestionarlo manualmente y sacar 'Pan' y
  // 'Select' del grupo automático si da problemas, pero Select y Pan son
  // ToolModes. Mejor: asignamos ID Pan al botón. Si estamos en Select, también
  // lo iluminamos? O mejor, asignamos ID Pan. UpdateToolStates iluminará este
  // botón si mode == Pan. Si mode == Select, tendremos que iluminarlo también.
  m_toolGroup->addButton(btnMode, static_cast<int>(ToolMode::Pan));
  // EL ID Select no tendrá botón en el grupo, lo gestionaremos en el switch de
  ...
  measureMenu->setStyleSheet(
      "QMenu { background-color: #1a1a1a; color: white; border: 1px solid "
      "#333; } QMenu::item:selected { background-color: #27ae60; }");

  QAction *actRuler = measureMenu->addAction(
      createWhiteIcon(":/resources/icons/rule_icon_bar.svg"), "Regla");
  QAction *actProtractor = measureMenu->addAction(
      createWhiteIcon(":/resources/icons/transportador_icon_bar.svg"),
      "Transportador");
  ...
  if (auto layout =
          ui->centralwidget->findChild<QVBoxLayout *>("toolbarVLayout")) {
    // Insertar después de los botones de dibujo, en la posición de
    // toolProtractor
  ...
  // === SELECCIÓN (NUEVO) ===
  connect(m_chartWidget->scene(), &QGraphicsScene::selectionChanged, this,
          &MainWindow::onSelectionChanged);
  connect(m_strokePopup, &StrokeSettingsPopup::toggleProjectionsRequested, this,
          &MainWindow::onToggleProjections);

  // === CONEXIÓN RADIO COMPÁS ===
  connect(m_strokePopup, &StrokeSettingsPopup::radiusChanged, this,
          [this](double radius) {
...
void MainWindow::setupConnections() {
  ...
  connect(ui->registerButton, &QPushButton::clicked, this, [this]() {
    ...
    QImage avatar;
    QLabel *preview =
        ui->registerPage->findChild<QLabel *>("avatarPreviewLabel");
    if (preview) {
  ...
  connect(m_profileView, &ProfileView::backRequested, this,
          &MainWindow::showDashboard);
  connect(m_resultsView, &ResultsView::backRequested, this,
          &MainWindow::showDashboard);

  // Register Avatar Button
  QPushButton *btn =
      ui->registerPage->findChild<QPushButton *>("selectAvatarButton");
  if (btn) {
      ...
      if (!fileName.isEmpty()) {
        ...
        if (!img.isNull()) {
          ...
          if (preview) {
            ...
            // For now we set it as a property on the preview label to retrieve
            // it later? Better: adding a member `m_tempRegisterAvatar` in
            // MainWindow if needed, or retrieve from pixmap (lossy). Let's use
            // property.
...
void MainWindow::setupRegisterValidation() {
  auto validateField = [](QLineEdit *edit, QLabel *errorLabel,
                          std::function<ValidationResult(QString)> validator) {
    QString text = edit->text();
    ValidationResult result = validator(text);
    if (result.valid) {
      edit->setStyleSheet(
          "border: 2px solid #27ae60; background-color: #2a2a2a; color: white; "
          "padding: 10px; border-radius: 8px;"); // Green
      errorLabel->setText("");
    } else {
      edit->setStyleSheet(
          "border: 2px solid #e74c3c; background-color: #2a2a2a; color: white; "
          "padding: 10px; border-radius: 8px;"); // Red
      errorLabel->setText(result.errorMessage);
    }
  };

  connect(ui->registerNickEdit, &QLineEdit::textChanged, this, [=]() {
    validateField(ui->registerNickEdit, ui->registerErrorLabel,
                  [](QString s) { return Validators::validateNick(s); });
  });

  connect(ui->registerEmailEdit, &QLineEdit::textChanged, this, [=]() {
    validateField(ui->registerEmailEdit, ui->registerErrorLabel,
                  [](QString s) { return Validators::validateEmail(s); });
  });

  connect(ui->registerPasswordEdit, &QLineEdit::textChanged, this, [=]() {
    validateField(ui->registerPasswordEdit, ui->registerErrorLabel,
                  [](QString s) { return Validators::validatePassword(s); });
  });
}
...
void MainWindow::onStartQuiz() {
  if (!m_loginController->isLoggedIn()) {
    QMessageBox::warning(this, "Acceso denegado",
                         "Debes iniciar sesión para realizar problemas.");
    return;
  }
...
void MainWindow::onSelectionChanged() {
  // Actualizar UI del popup si estamos en modo selección o si simplemente hay
...
void MainWindow::toggleLoginPasswordVisibility() {
...
void MainWindow::toggleRegisterPasswordVisibility() {
...
void MainWindow::toggleRegisterConfirmPasswordVisibility() {
